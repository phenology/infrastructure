...
object satellite_model_svd_func extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    var out_path = "hdfs:///user/emma/svd/spark/" + model_dir + satellite_dir + "Rhode" + n_components + "/"

    //Years between (inclusive) 1989 - 2014
    val satellite_timeseries = (1980, 2016)
    ...
    //Load Mask
    def load_mask(mask_path:String) :Tile = {
      val mask_tiles_RDD = sc.hadoopGeoTiffRDD(mask_path).values
      val mask_tiles_withIndex = mask_tiles_RDD.zipWithIndex().map { case (e, v) => (v, e) }
      val mask_tile = (mask_tiles_withIndex.filter(m => m._1 == 0).filter(m => !m._1.isNaN).values.collect()) (0)
      mask_tile
    }

    if (modToBeMasked) {
      mod_mask_tile0 = load_mask(mod_mask_path)
    }

    if (satToBeMasked) {
      sat_mask_tile0 = load_mask(sat_mask_path)
    }
    ...
    def hadoopGeoTiffRDD(satellite_filepath :String, pattern :String): RDD[(ProjectedExtent, Tile)] = {
      val listFiles = sc.binaryFiles(satellite_filepath + "/" + pattern).sortBy(_._1).keys.collect()
      var prevRDD :RDD[(ProjectedExtent, Tile)] = sc.emptyRDD

      cfor(0)(_ < listFiles.length, _ + 1) { k =>
        val filePath :String = listFiles(k)
        val currRDD = sc.hadoopGeoTiffRDD(filePath)
        if (prevRDD.isEmpty()){
          prevRDD = currRDD
        } else {
    ...
    def hadoopMultibandGeoTiffRDD(satellite_filepath :String, pattern :String): RDD[(ProjectedExtent, MultibandTile)] = {
      val listFiles = sc.binaryFiles(satellite_filepath + "/" + pattern).sortBy(_._1).keys.collect()
      var prevRDD :RDD[(ProjectedExtent, MultibandTile)] = sc.emptyRDD

      cfor(0)(_ < listFiles.length, _ + 1) { k =>
        val filePath :String = listFiles(k)
        val currRDD = sc.hadoopMultibandGeoTiffRDD(filePath)
        if (prevRDD.isEmpty()){
          prevRDD = currRDD
        } else {
          prevRDD = currRDD.union(prevRDD)
    ...
    def loadGeoTiffsTileAsRDD(path :String, pattern :String, band :Int, toBeMasked :Boolean, maskPath :String, maskCellTrue :Int, maskCellVal :Int): RDD[Array[Double]] = {
      var grids_RDD :RDD[Array[Double]] = sc.emptyRDD
      var tiles_RDD :RDD[Tile] = sc.emptyRDD
      if (band == 0) {
        val geos_RDD = hadoopGeoTiffRDD(satellite_filepath, pattern)
        tiles_RDD = geos_RDD.values
      } else {
        val band_numB: Broadcast[Int] = sc.broadcast(sat_band_num)
        val geos_RDD = hadoopMultibandGeoTiffRDD(satellite_filepath, pattern)
        tiles_RDD = geos_RDD.values.map(m => m.band(band_numB.value))
      }
      if (toBeMasked) {
        val mask_tile = load_mask(maskPath)
        val tile_broad: Broadcast[Tile] = sc.broadcast(mask_tile)
        grids_RDD = tiles_RDD.map(m => m.localInverseMask(tile_broad.value, maskCellTrue, maskCellVal).toArrayDouble())
      } else {
    ...
    if (satellite_rdd_offline_mode) {
      ...
      sat_grids_RDD = loadGeoTiffsTileAsRDD(satellite_filepath, pattern, sat_band_num, satToBeMasked, sat_mask_str, 1, -2000)
    ...
    val matrix_mul_QR = new IndexedRowMatrix(matrix_mul.rows.sortBy(_.index)).toRowMatrix().tallSkinnyQR(true)
    val svd: SingularValueDecomposition[RowMatrix, Matrix] = matrix_mul_QR.Q.computeSVD(n_components.toInt, true)
